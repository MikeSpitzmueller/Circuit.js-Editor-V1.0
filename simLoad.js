...
function didUpdate(sim) {
  ...
  // console.log(new ModuleManager().getModuleList())
  ...
  var moduleList = new ModuleManager().getModuleList();
    // console.log(moduleList[0].output);

  if(moduleList[0] !== undefined) {
    if(moduleList[0].classname == "generator_sin") {
      // console.log(moduleList[0]);


      if(moduleList[0].output.length == 0) {
        freq = 0;
        ampl = 0;
      }
      else if (moduleList[0].output[0].output !== 0) {
        // if(moduleList[0].output[0].output.classname == "circuit") {
        // console.log(moduleList[0])
        freq = moduleList[0].frequency;
        ampl = moduleList[0].amplitude;
      // }
      }
...
function didStep(sim) {
  var t = sim.getTime();
  var q1 = ampl*Math.sin(freq*Math.PI*2*t);
  var q2 = ampl*Math.cos(freq*Math.PI*2*t);
  var q3 =  4;
  var q4 = 10;
  document.dispatchEvent(new CustomEvent('DidSimStep',            //event wird für jeden schritt ausgelöst
  {
    detail:
      {
        time: t,
...
function simLoaded() {
  ...
  console.log(iframe.contentWindow.oncircuitjsloaded);
  console.log(sim.isRunning());

}

// function frequencySelect() {
//   freq = parseFloat(document.getElementById("freq_select").value);
// }
